...
This document defines the descriptor direction for `rustos`.
...
- one small ownership sketch
- one small summary helper

For the current stage, a descriptor-like handle can be treated as:
...
For the current stage, a small typed wrapper is enough.
...
It is only the smallest useful rule for the current stage.

## Ownership Direction

The descriptor boundary should eventually support clear ownership rules.

For the current stage, the important direction is:
...
This is enough to make the task-to-descriptor relationship slightly more concrete without introducing:
...
The current ownership sketch should stay simple:
...
For the current stage, release behavior should remain conceptual only.
...
- the tiny ownership sketch should not imply per-task descriptor tables
- task lifecycle should not imply descriptor inheritance or cleanup rules
- those rules should be introduced only when task and syscall behavior become more concrete

This keeps the current direction focused.
...
This avoids pulling too much design into this stage.
...
For the current `rustos` direction, the descriptor boundary is:
