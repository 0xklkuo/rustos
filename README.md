...
Current milestone:
- Milestone U4 — real memory foundation and first conventional-range frame allocator seed groundwork
...
- the current memory milestone validates real UEFI memory-map discovery and now extends that groundwork with a minimal frame allocator seed derived from the first discovered conventional memory range, while paging and heap support remain deferred.
